...
> Estado: **propuesta v3, pendiente de aprobación**. Depende de la Etapa 1 aprobada (`etapa-1-analisis-producto.md`). Decisiones confirmadas: modelo de fondos **split payment** (el dinero va directo al organizador vía la pasarela; Juntalo solo cobra comisión) y backend en **Go**. Nota de historial: se evaluó NestJS full-TS (v2, ver historial git) y también Firebase/Firestore; el usuario decidió volver a Go priorizando rendimiento y huella operacional en el VPS, asumiendo conscientemente la curva de aprendizaje del lenguaje y el mantenimiento manual del contrato API (mitigable con OpenAPI codegen, ver §2.5).
...
- **Contrato API tipado hacia el frontend**: al no compartir lenguaje con el frontend (trade-off asumido de Go vs full-TS), el contrato se formaliza con **OpenAPI**: anotaciones/spec en el backend → `openapi-typescript` genera los tipos TS que consume el cliente HTTP del frontend. Paso de codegen en `make check`/CI para que el contrato nunca se desincronice silenciosamente.
