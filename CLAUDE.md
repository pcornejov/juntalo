...
- **Transacciones financieras**: `PaymentRepo.ConfirmByProviderRef` bloquea la fila con `FOR UPDATE` dentro de `pgx.BeginFunc` y transiciona pago + contribution atómicamente. Juntalo es solo para aportar a causas — no hay reembolso self-service, un pago confirmado es terminal.
...
Construido: registro/login (email+password y Google, vacío `GOOGLE_CLIENT_ID` = sin botón de Google), verificación de email, recuperación de contraseña, creación/edición/publicación de campañas (tipo "collection" y "raffle" habilitados), página pública con OG tags + QR + compartir WhatsApp, flujo de aporte público con pago mock/Webpay real, dashboard de organizador (participantes, export CSV, totales, costo de servicio), notificaciones por email al organizador, buscador/filtro (campañas y participantes), clonar campaña, auto-publicar en fecha programada (scheduler en background, tick cada minuto), dark mode, monitoreo de errores con Sentry, comisión editable por organización desde el backoffice, sistema de liquidación manual (datos de transferencia del organizador + backoffice "pendiente de liquidar" + marcar como pagado). No hay reembolsos: Juntalo es solo para aportar a causas, un aporte confirmado es definitivo.
